...
const siteIsTextNow = window.location.href.startsWith('https://www.textnow.com/messaging');
...
const selectors = {
    ...
    // TextNow selectors
    tnMessageBubble: '.chat-item.outgoing .message-text',
    tnMessageEditor: '#text-input',
...
function findTextNow() {
    ...
    // Wait for the Start Chat button to load before starting the process to send the text
    waitForElementToLoad(selectors.tnStartChatButton)
        .then(() => {
            console.log('configuring TextNow site');
            siteManager = new TextNowSiteManager();
            siteManager.initialize();
        })
        .catch((err) => {
            console.error(err);
...
async function chooseTextPlatform() {
    ...
    if (textPlatform === 'text-now') findTextNow();
}
